...

{
	...
	my $dbname = "Rulebase2011DB";	#	database name 	
	...
	sub readFile
	{
		...
		while (not $err)
		{
			...
			my $dbh=DBI->connect ($dsn,$username,$password,\%conn_attrs)
				or die "Cannot connect to database: $DBI::errstr. Please make sure information".
					   " between line 43 and 46 of file Main.pm is correct for database access.";
	...
	sub applyRules 
	{
		...
		if ($no_exec) { return ''; }
			...
			{ return $err; }
			#	printf "Applying transport rules Done.\n";
		...
		return $model->write($fname,$write_sbml);
	}
	...
	sub write{
		...
		#	Write model in a native format
		$out .= "# Created by Rulebase2011, authored by Chen Liao, USTC\n";
		$out .= $model->ParamList->write();	#	parameters
		$out .= $model->CompartmentList->write();	#	compartments
		$out .= $model->SpeciesList->write($model->CompartmentList);	#	species
		#	$out .= $model->FunctionList->write();	#	functions
		$out .= $model->EventList->write();	#	events
		$out .= $model->RuleList->write($model->SpeciesList);	#	rules
		$out .= $model->ReactionList->write($model->SpeciesList);	#	reactions
